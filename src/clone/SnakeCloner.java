...
public class SnakeCloner {
  ...
  public static ArrayList<Snake> clone(ArrayList<Snake> snakes) {
    ...
    Snake snake1 = new Snake(snakePoints1.toArray(new Point[snakePoints1.size()]),
        cloned.getDir().getOpposit());
    Snake snake2 = new Snake(snakePoints2.toArray(new Point[snakePoints2.size()]),
        cloned.getDir().getDir());
    if (snakes.size() < 3) {
      snakes.add(snake1);
    }
    snakes.add(snake2);
